...
class NetworkTask extends AsyncTask<String, Place, ArrayList<Place>>
{
	...
	protected ArrayList<Place> doInBackground(String... searchquery)
	{
		...
		toReturn = queryJettyServer(searchquery[0], searchquery[1], searchquery[2]);
	...
	public ArrayList<Place> queryJettyServer(String locationFilter, String radiusFilter, String typeFilter)
	{
		...
		try {
			...
			// Now that we have connected to the server we want to query our Jetty server
			...
			Log.i("!", "HTTPClient");
		    String queryRequestURL = "http://" + MainActivity.IP + ":" + MainActivity.Port + "/data?format=xml&location=" + locationFilter2.replaceAll("\n", "") + "&radius=" + radiusFilter.replaceAll("\n", "") + "&type=" + typeFilter.replaceAll("\n", "");
		    Log.i("!", "HTTP URL");
		    ...
		    Log.i("!", finalXMLStringResultToParse);
	...
	public static void updateUINoResults()
	{
		...
		MapActivity.resultingPlaces_.append("There were no results, please try again.");
